...
function reactionFx(kind,icon){
 ...
 if(!reducedMotion())for(let n=0;n<10;n++){const p=document.createElement('i');p.style.setProperty('--x',Math.round(Math.cos(n*Math.PI/5)*94)+'px');p.style.setProperty('--y',Math.round(Math.sin(n*Math.PI/5)*66)+'px');p.style.setProperty('--turn',n*36+'deg');p.style.setProperty('--delay',n*14+'ms');box.appendChild(p)}
...
function start(m,i){if(i<0||i>49)return;clearFeedback();completed=false;mode=m;level=i;sel=null;errors=0;runXp=0;$('runXp').textContent=0;$('retryBtn').textContent=S.lang==='en'?'↻ Restart level':'↻ Začať level znova';const t=tr(),a=t.mode[m];$('playTitle').textContent=a[1];$('playSub').textContent='Level '+(i+1)+' / 50';$('goalKicker').textContent=t.goal;$('msg').className='msg';$('msg').textContent=m==='life'?t.odd:t.combine;if(m==='life'){const L=LIFE[i];lifeOdd=L.odd;board=L.items.slice();$('goalText').textContent='🧠 '+(S.lang==='en'?'Odd one out':'Čo sem nepatrí?');$('goalHint').textContent=(S.lang==='en'?'Three belong to: ':'Tri veci patria do skupiny: ')+L.rule[S.lang==='en'?1:0];renderLife()}else{const L=(m==='chem'?CHEM:ADV)[i];board=L.tiles.slice();$('goalText').innerHTML=iconMarkup(I[L.goal][0])+' '+nm(L.goal);$('goalHint').textContent=t.combine;renderAlchemy()}show('play')}
function renderAlchemy(){const box=$('tiles');box.innerHTML='';board.forEach((id,i)=>{const b=document.createElement('button');if(!id){b.className='tile empty';b.disabled=true}else{b.className='tile'+(sel===i?' sel':'');b.setAttribute('aria-pressed',sel===i?'true':'false');b.innerHTML='<span class="e">'+iconMarkup(I[id][0])+'</span><span class="n">'+nm(id)+'</span><span class="tag">'+tr().select+'</span>';b.onclick=()=>tap(i)}box.appendChild(b)})}
